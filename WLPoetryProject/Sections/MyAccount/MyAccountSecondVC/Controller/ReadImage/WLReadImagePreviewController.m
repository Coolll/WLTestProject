...
@interface WLReadImagePreviewController ()
/**
 *  图片
 **/
 ...
 *  保存图片的block
 **/
@property (nonatomic,copy) SureSelectImageBlock saveBlock;


@end

@implementation WLReadImagePreviewController
...
- (void)setImageColorString:(NSString *)imageColorString{
    _imageColorString = imageColorString;
    NSArray *array = [imageColorString componentsSeparatedByString:@","];
    self.imageColor = [UIColor whiteColor];
    if (array.count == 3) {
        NSInteger red = [[array objectAtIndex:0] integerValue];
        NSInteger green = [[array objectAtIndex:1] integerValue];
        NSInteger blue = [[array objectAtIndex:2] integerValue];
        self.imageColor = RGBCOLOR(red, green, blue, 1);
    }
...
- (void)loadMainBackImageView
{
    ...
    if (self.type == PreviewTypeImage) {
        ...
        self.mainImageView.backgroundColor = self.imageColor;
    }
...
- (void)saveAction:(UIButton*)sender{
    ...
    //采取覆盖模式
    if (self.type == PreviewTypeImage) {
        [WLSaveLocalHelper saveReadImageURLOrBackgroundRGB:self.imageName];
    }else{
        [WLSaveLocalHelper saveReadImageURLOrBackgroundRGB:self.imageColorString];
    }
